...
use app\models\mgcms\db\ModelAttribute;
...
class AbstractRecord extends \yii\db\ActiveRecord
{
  ...
  public function save($runValidaton = true, $attributes = null)
  {
    ...
    try {
      ...
      Log::logError('db', $error);
      MgHelpers::setFlashError('Błąd zapisu, sprawdź poprawność danych');
      return false;
    }
    if ($saved && !$isNewRecord) {
      $this->saveModificationHistory($oldAttributes);
    }
    return $saved;
  }
  ...
  {
    $this->assignRelatedFiles();
    return parent::beforeSave($insert);
  }

  private function assignRelatedFiles()
  {

    if (isset($_FILES) && isset($_FILES[MgHelpers::getClassShortName($this)]) && isset($_FILES[MgHelpers::getClassShortName($this)]['name'])) {
      foreach (array_keys($_FILES[MgHelpers::getClassShortName($this)]['name']) as $attribute) {
        if ($this->hasAttribute($attribute)) {
          $upladedFile = UploadedFile::getInstance($this, $attribute);
          if ($upladedFile) {
            if ($upladedFile->hasError) {
              ...
              if ($file->id) {
                $this->setAttribute($attribute, $file->id);
              }
            }
          }
        }
...
//      MgHelpers::setPrettyPhoto();
    return isset($this->$field) && $this->$field && is_object($this->$field) && method_exists(get_class($this->$field), 'getThumb') ? $this->$field->getThumb($width,
            $height) : false;
  }
...
//            });
  ...
  {
    $skippedRelations = \yii\helpers\ArrayHelper::merge($skippedRelations, ['files', 'fileRelations', 'tags', 'roles']);
  ...
  private function saveModificationHistory($oldAttributes)
  {
    if (self::className() != 'app\models\mgcms\db\ModificationHistory' && isset($this->id)) {
      $model = new ModificationHistory;
      $model->model_class = self::className();
      $model->model_id = $this->id;
      $model->model = serialize($this->getAttributes());
      $model->previous_model = serialize($oldAttributes);
      $model->save();
    }
  }
}
