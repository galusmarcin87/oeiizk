...
class MgHelpers extends \yii\base\Component
{
  ...
  public static function getSetting($name, $textValue = false, $defaultValue = '')
  {
    $setting = Setting::find()->cache(36000)->where(['key' => $name])->one();
  ...
  {
    $arr = [];
    foreach (Yii::$app->getSession()->allFlashes as $flType => $flash) {
      if ($flType == $type) {
        array_push($arr, $flash[0]);
      }
    }
    array_push($arr, $msg);

    Yii::$app->session->setFlash($type, $arr);
//    Yii::$app->session->setFlash($type, $msg);
  ...
  {
    self::setFlash(self::FLASH_TYPE_INFO, $msg);
  }

  /**
   * pobiera tablice wartoßci oddzielonych przecinkami
   */
  public static function getSettingsArray($name)
  {
    return explode(',', MgHelpers::getSetting($name));
  }
  ...
  {
    Yii::error(get_class($model) . ', Save error: ' . MgHelpers::getErrorsString($model->getErrors()) . ' object: ' . \yii\helpers\Json::encode($model),
        'error', 'system.log');
  }
  ...
  public static function saveModelAndLog($model, $flash = true)
  {
    ...
    if (!$saved) {
      if ($flash) {
        MgHelpers::setFlash(MgHelpers::FLASH_TYPE_ERROR,
            T::sbt('Error saving') . ' ' . T::sbt(get_class($model)) . ':' . MgHelpers::getErrorsString($model->getErrors()));
      }
   ...
   * 
   */
  public static function getModelAttributeHtmlId($model, $attribute)
  {
    return strtolower(self::getClassShortName($model) . '-' . $attribute);
  }
  ...
  public static function registerJsFile($src, $options = [], $module = false)
  {
    if ($module) {
  ...
  public static function registerCssFile($src, $options = [], $module = false)
  {
    if ($module) {
      $src = Yii::$app->getModule($module)->basePath . $src;
    }
    $files = Yii::$app->view->getAssetManager()->publish($src);
    if (isset($files[1])) {
      Yii::$app->view->registerCssFile($files[1]);
    } else {
      self::log('error with publishing js file ' . $src);
    }
  }
  ...
  {
    return Yii::$app->getSecurity()->decryptByPassword(self::base64_url_decode($hash),
            \app\components\mgcms\MgHelpers::getConfigParam('secretKey'));
  }
  ...
  public static function getCRUDColumn($controller = false)
  {
    return [
  ...
  public static function isAdmin()
  {
    ...
    if ($user && $user->role == \app\models\mgcms\db\User::ROLE_ADMIN) {
      return true;
    } else {
      return false;
    }
  ...
  public static function getClassShortNameByName($className)
  {
    return (substr($className, strrpos($className, '\\') + 1));
  }

  public static function dropdownDataFromSettings($name)
  {
    return MgHelpers::arrayTranslateValues(MgHelpers::getSettingOptionArray($name));
  }
  ...
  public static function isExistModelInModelsArrayById(Array $models, int $id)
  {
    foreach ($models as $model) {
      if ($model->id == $id) {
        return true;
   ...
   */
  static function checkIfDateOverlap($from1, $to1, $from2, $to2)
  {
    if (!is_numeric($from1)) {
  ...
  static function backendGetSwitchColumnSettings($label)
  {
    return ['type' => \kartik\builder\TabularForm::INPUT_WIDGET,
  ...
  static function backendGetCheckboxColumnSettings($label)
  {
    return ['type' => \kartik\builder\TabularForm::INPUT_CHECKBOX,
            'options' => [
                'uncheck' => 0,
